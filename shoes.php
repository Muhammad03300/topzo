...
$categoryFilter = isset($_GET['product_category']) ? $_GET['product_category'] : 'Shoes';

// Fetch products from the database based on the category filter
if ($categoryFilter == 'All') {
    $query = "SELECT * FROM product WHERE LOWER(product_category) IN ('shoes', 'sandals')";
} else {
...
<div class="container mt-5">
    ...
    <form action="shoes.php" method="get" class="mb-3">
        <div class="form-group">
            ...
            <select class="form-control" id="categoryFilter" name="product_category">
                ...
                <option value="Shoes" <?php echo ($categoryFilter == 'Shoes') ? 'selected' : ''; ?>>Shoes</option>
                <option value="Sandals" <?php echo ($categoryFilter == 'Sandals') ? 'selected' : ''; ?>>Sandals</option>
            </select>
        </div>
