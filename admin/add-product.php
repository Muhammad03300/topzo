...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["file_path"]) && $_FILES["file_path"]["error"] == 0) {
        ...
        if ($fileUrl) {
            ...
            $stmt = $conn->prepare("INSERT INTO products (product_title, product_description, product_category, google_product_category, stock_availability, regular_price, product_condition, manufacturer, mpn, shipping, adult, age_group, color, gender, size, material, pattern, item_group_id, file_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
